...
! The first piece will be the force with respect to atom 3.
                if (x .gt. 1.0d-5) then
                  ...
                  deallocate (f3naXb)
        deallocate (f3naXc)      else
                  amt = 0.0d0
                end if

                pFdata_bundle => Fdata_bundle_3c(in1, in2, in3)
                pFdata_cell =>                                                 &
     &          pFdata_bundle%Fdata_cell_3c(pFdata_bundle%index_3c(interaction,&
     &          isorp,1))
...
! Force on the neutral atom with respect to atom 3 (f3naMa).
              call Drotate (in1, in2, eps, depsA, norb_mu, norb_nu, bcnam,     &
        &          f3naMa, f3naXa)

! Force on the neutral atom with respect to atom 1 (f3naMb).
              call Drotate (in1, in2, eps, depsB, norb_mu, norb_nu, bcnam,     &
        &          f3naMb, f3naXb)
