...
! Determine if the abs.inp files exists - if not, then exit.
        ...
        inquire (file = slogfile, exist = read_density_matrix)
        if (read_density_matrix) then
! Read from input file - gives dos options
          write (logfile,*) ' Reading from pre-existing density matrix file! '
          open (unit = inpfile, file = slogfile, status = 'old', form = 'unformatted')
          ! loop over the atoms.
          do iatom = 1, s%natoms
            ...
            do ineigh = 1, num_neigh  ! <==== loop over i's neighbors
              ...
              read (inpfile) s%denmat(iatom)%neighbors(ineigh)%block
            end do ! end loop over neighbors
          end do ! end loop over atoms
          close (inpfile)
        else

! Generate the initial density matrix and write out to a file.
          write (logfile,*) ' Initializing the density matrix! '
          open (unit = inpfile, file = slogfile, status = 'new', form = 'unformatted')
          ! loop over the atoms.
          do iatom = 1, s%natoms
            ! cut some lengthy notation
            pdenmat=>s%denmat(iatom)
            ...
            do ineigh = 1, num_neigh  ! <==== loop over i's neighbors
              ...
              ! building the initial density matrix
              if (iatom .eq. jatom) then
                imu = 1
                do issh = 1,species(in1)%nssh
                  nlmu = (2*species(in1)%shell(issh)%lssh + 1)
                  do lmu = 1, nlmu
                    pRho_neighbors%block(imu,imu) = s%atom(iatom)%shell(issh)%Qneutral*(1.0d0/nlmu)
                    imu = imu + 1
                  end do ! do lmu
                end do ! do issh
              end if
